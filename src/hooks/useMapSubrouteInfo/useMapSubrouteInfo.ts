...
type SubrouteInfoArray = [ViewValue, ...Array<string | null>];

// Use under Map/ route to acquire subroute view, geography, geoid info
export const useMapSubrouteInfo = ():SubrouteInfoArray | null => {
  const router = useRouter();

  const { subroutes } = router.query;

  const [viewParam, geographyParam, geoidParam] = subroutes ? subroutes : [null, null, null];

  const view = typeof viewParam === "string"
  ...
  const geography = typeof geographyParam === "string"
    ? geographyParam
